...
\begin{itemize}[leftmargin=2em]
  \item \textbf{实时快照}：每条消息发送成功并写入数据库后，服务器立即重新计算 Root，将\texttt{MERKLE\_ROOT\_UPDATED}记录写入\texttt{audit\_logs}；
  \item \textbf{管理员手动快照}：管理员可通过管理端接口手动触发某订单的 Merkle Root 快照；
  \item \textbf{历史补全快照}：管理员可对所有尚未生成快照的历史订单批量补全 Root 快照。
\end{itemize}
...
\input{content/chapter3/figures/fig_merkle_verify}
