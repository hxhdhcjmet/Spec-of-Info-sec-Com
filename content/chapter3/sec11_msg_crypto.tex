...
\input{content/chapter3/figures/fig_msg_pipeline}
...
管理员查询消息记录时，\texttt{/admin/messages/\{order\_id\}} 接口仅返回 \texttt{msg\_id}、\texttt{order\_id}、\texttt{sender\_pid}、\texttt{role}、\texttt{message\_hash} 和 \texttt{timestamp} 字段，不返回 \texttt{content} 密文字段。Merkle 完整性验证只需 \texttt{message\_hash} 列表，管理员无需接触消息密文即可完成日志验证，实现审计与明文访问的权限隔离。
