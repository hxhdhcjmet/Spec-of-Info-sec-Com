...
\input{content/chapter3/figures/fig_register}
...
\[
  h_{\mathrm{pwd}} = \operatorname{SM3}(s \| \mathit{password})
\]
数据库中仅保存 $\langle s,\, h_{\mathrm{pwd}}\rangle$，原始口令在计算完成后即丢弃。登录时，服务器从数据库读取 $s$，对用户提交的口令重新计算哈希，并使用恒定时间比较函数（\texttt{hmac.compare\_digest}）与 $h_{\mathrm{pwd}}$ 对比，避免时序侧信道攻击。
