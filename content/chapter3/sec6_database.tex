...
如表~\ref{tab:db_users}、表~\ref{tab:db_orders}、表~\ref{tab:db_messages}和表~\ref{tab:db_audit}所示，
系统数据库由 \texttt{users} 用户表、\texttt{orders} 订单表、\texttt{messages} 消息表和
\texttt{audit\_logs} 审计日志表组成。四张表之间通过外键关联，整体以订单号为核心索引构建数据流。\texttt{users} 与 \texttt{orders} 通过 \texttt{user\_id} 关联，\texttt{orders} 与 \texttt{messages} 通过 \texttt{order\_id} 关联，\texttt{audit\_logs} 通过 \texttt{order\_id} 与订单和消息建立审计关系。
