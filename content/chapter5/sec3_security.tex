...
\textbf{S10--S11：完整性与溯源}。S10 验证篡改消息密文后完整性验证返回 \texttt{VERIFY\_FAIL}（\texttt{hash\_mismatch\_count=1}，\texttt{root\_match=False}），详见 5.4 节。S11 验证管理员无法绕过审批流程直接扫描明文，\path{/admin/trace_violation} 接口返回 403，溯源必须经完整性校验和审批双重门槛。
